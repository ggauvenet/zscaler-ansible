...
logger = logging.getLogger('ansible-zpa-server')

CONFIG_SPACE = ["DEFAULT", "SIEM"]

def compare_server(req, get):
    equal = True
    test = {}
    for key in req.keys():
        if key in get:
...
def main(): 
    ...
    module = AnsibleModule( 
        argument_spec=dict( 
            ...
            configSpace = dict(type='str', default="DEFAULT", choices=CONFIG_SPACE),
        ),
    ...
    retlist = zcls.get('server', { 'search': 'name LIKE {0}'.format(name) })
    if 'list' in retlist:
    ...
    if _update and (get != None):
        (equal, test) = compare_server(req, get)
        if equal:
            action = "Checked"
        else:
            req['id'] = get['id']
    ...
    try: 
        if _create:
            ...
            if not module.check_mode:
                zcls.post('server', req)
            changed = True
            action = "Create"
        if _update and (not equal):
            #module.fail_json(msg="Update", get=get, req=req)
            if not module.check_mode:
                result = zcls.put('server/{0}'.format(get['id']), req)
            changed = True
            action = "Update"
        if _delete: 
            if not module.check_mode:
                zcls.delete('server/{0}'.format(get['id']))
            action = "Delete"
            changed = True
